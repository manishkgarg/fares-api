...
public class FareServiceTest {
	...
	@Before
	public void setup() {
		ReflectionTestUtils.setField(fareService, "resource", "http://localhost:8080/fares", String.class);
		...
		when(this.tokenGenerator.generateToken()).thenReturn("valid token");
	...
	public void mockFareService() throws Exception {
		when(this.webClient.get().uri("http://localhost:8080/fares/yow/bba").header("Authorization", "valid token")
				.accept(MediaType.APPLICATION_STREAM_JSON).retrieve()).thenReturn(createResponseSpec(HttpStatus.OK));
		Mono<FareModel> result = fareService.retrieveRates("yow", "bba");
		assertNotNull("result must not be empty", result);
	}

	private WebClient.ResponseSpec createResponseSpec(HttpStatus httpStatus) throws Exception {
